...
const Templates = () => {
    ...
    const handleTableUpdate = (e, id, amount) => {
        setRows((prevRows) => {
            ...
            const currentRowIndex = updateTable.findIndex(
                (row) => row.id === id
            );
            ...
            if (e.target.name === "rate" || e.target.name === "quantity") {
                updateTable[currentRowIndex][e.target.name] = Number(
                    e.target.value
                );
            }
            return updateTable;
    ...
    const handleJsonDownloadButton = () => {
        const jsonService = new JsonService();

        jsonService.downloadJson(formData, rows);
    };
    ...
    function fillFormData(uploadedFormData) {
        const tableRowData = [...uploadedFormData.tableRows];

        delete uploadedFormData.tableRows;
